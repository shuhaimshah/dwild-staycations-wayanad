      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6">
          {features.map((item) => (
            <div
              key={item.num}
              className="group bg-dwild-dark/60 border border-dwild-sand/15 p-8 transition-all duration-500 hover:border-dwild-sand hover:-translate-y-1 flex flex-col justify-between"
            >
              <div>
                <span className="text-2xl font-light text-dwild-sand/40 group-hover:text-dwild-sand transition-colors block mb-6">
                  {item.num}
                </span>
              ...
              <div className="mt-8 pt-4 border-t border-dwild-sand/10">
                <span className="text-[10px] tracking-[0.25em] text-dwild-sand uppercase opacity-0 group-hover:opacity-100 transition-opacity">
                  DWILD ESSENCE →
                </span>
              </div>
            </div>
