    ...
    <section className="py-28 bg-dwild-dark text-dwild-offwhite border-t border-dwild-sand/10">
      <div className="max-w-7xl mx-auto px-6 md:px-12">
          ...
          <AnimatePresence>
            {filteredExperiences.map((exp, idx) => (
              <motion.div
                ...
                whileHover={{ y: -6 }}
                className="group relative bg-dwild-jungle border border-dwild-sand/15 overflow-hidden h-[420px] flex flex-col justify-end p-8 transition-all duration-700 hover:border-dwild-sand hover:shadow-2xl rounded-sm"
              >
                {/* Card Image */}
        ...
        <ScrollReveal delay={0.2} className="mt-12 text-center">
          <Link
            to="/experiences"
            className="group inline-flex items-center gap-3 text-xs tracking-[0.25em] uppercase font-semibold text-dwild-sand border-b border-dwild-sand pb-1 hover:text-dwild-offwhite hover:border-dwild-offwhite transition-colors"
          >
